    ...
    if (typeof newUser !== "string" || newUser === "") {
        ...
        usersCollection.findOne({ username: newUser }, (err, data) => {
            if (err) {
                ...
                usersCollection.insertOne({ username: newUser.trim(), log: [], count: 0 }, (err, data) => {
                    if (err) {
                        res.status(500).json({ "error": "Something went wrong, please try later." })
                    } else if (data) {
...
app.post("/api/exercise/add", function(req, res) {
    ...
    if (!ObjectID.isValid(userId)) {
        ...
        usersCollection.updateOne({ _id: ObjectID(userId) }, {
            $push: {
                log: {
        ...
        usersCollection.find({ _id: ObjectID(userId) }).toArray((err, data) => {
            if (err) {
                ...
                let result = data.reduce((acc, item, idx) => {
                    ...
                    let fr = from && new Date(from).toDateString();
                    let t = to && new Date(to).toDateString();
                    return {
                        username: item.username,
                        log: logs,
                        count: logs.length,
...
// Hint
// This is the expected output of the 4th test:
const expected = {
    username: 'testUser4', // Obviously the numbers change
    description: 'test',
    duration: 60,
    _id: "5ffae94d23d7b303f0f69f", // Example id 
